...
#'     the resulting area size. Default is 1.
#'
...
calculate_area <- function(d, digits = 1) {
  tri.area <- round(tan(30 * (pi / 180)) * (9 / 4) * d ^ 2, digits = digits)
  hex.area <- round(((3 * sqrt(3)) / 2) * d ^ 2, digits = digits)
